...
$(function(){
	...
	var transcriptList = '${transcriptList}'.split(',');
	$("#searchTranscript").autocomplete(transcriptList, {width: "9em", delay:50, minChars:0, autoFill:false, matchContains: true, cacheLength:0})
	.result(function() {
		searchTranscript($("#searchTranscript").val(), "${subset1}", "${subset2}");
	});
});
...
function searchGene(geneName, subset1, subset2) {
	...
	window.location.href = "right_searchGene.jsp?subset1=" + subset1 +"&subset2=" + subset2 + "&geneName=" + geneName;
}

function searchTranscript(transcript, subset1, subset2) {
	$("#searchTranscript", parent.document).block({ message: "<div>Loading...<br>Please Wait ...</div>"});	
	
	window.location.href = "right_searchGene.jsp?subset1=" + subset1 +"&subset2=" + subset2 + "&geneName=&transcript=" + transcript;
}
</script>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-bottom: 10px;">
	...
	<tr>
		...
		<td id="td_text_left" style="font-weight: bold; padding-left: 1px;">
			...
			&nbsp;
			&nbsp;
			<input type="text" id="searchTranscript" name="searchTranscript" placeholder="Enter Transcript ID" value="${transcript}" style="width: 12em" />
			&nbsp;
			&nbsp;
			&nbsp;
			<a href="right_searchGene_download.jsp?subset1=${subset1}&subset2=${subset2}&geneName=${geneName}&transcript=${transcript}" target="_blank" style="text-decoration:none;">
