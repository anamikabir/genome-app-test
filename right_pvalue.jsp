...
if (pValue == null || pValue.length() == 0) {
	pValue = "1E-6";
	gseInfoList = subsetExpService.select(subset1, subset2, pValue, type);
...
$(function(){
	...
	if($("#expTable tr").length == 4)
		window.location.href = "right_pvalue.jsp?subset1=${subset1}&subset2=${subset2}&isUpRegulation=${!isUpRegulation}&pValue=${pValue}&type=${type}";
});
...
function changePValue(target, subset1, subset2, isUpRegulation, type) {
	...
	window.location.href = "right_pvalue.jsp?subset1=" + subset1 +"&subset2=" + subset2 + "&isUpRegulation=" + isUpRegulation + "&pValue=" + $(target).val() + "&type=" + type;
}

function changeRegulation(target, subset1, subset2, pValue, type) {
	$("#pvalue", parent.document).block({ message: "<div>Loading...<br>Please Wait ...</div>"});	
	window.location.href = "right_pvalue.jsp?subset1=" + subset1 +"&subset2=" + subset2 + "&isUpRegulation=" + $(target).val() + "&pValue=" + pValue + "&type=" + type;
}
...
function changePage(e, target, subset1, subset2, isUpRegulation, pValue, type) {
	...
	if (keynum == 13 && re.test(v)) {
		...
		var end = index * 20 + 20;
		window.location.href = "right_pvalue.jsp?subset1=" + subset1 +"&subset2=" + subset2 + "&isUpRegulation=" + isUpRegulation + "&pValue=" + pValue + "&start=" + start  + "&end=" + end + "&type=" + type;
	}
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-bottom: 10px;">
	...
	<tr>
		...
		<td id="td_text_left" style="font-weight: bold; white-space: nowrap;">
			...
			&nbsp;
			&nbsp;
			<a href="right_pvalue_download.jsp?subset1=${subset1}&subset2=${subset2}&isUpRegulation=${isUpRegulation}&pValue=${pValue}&start=${start}&end=${end}&type=${type}" target="_blank" style="text-decoration:none;">
		...
		<td width="30%" id="td_text_right">
			<table cellpadding="0" cellspacing="0" border="0" style="display: inline;position: fixed; top: 0px; right: 5px;">
				...
				<tr>
					<td id="td_text_left" colspan="8" style="font-size:12px;font-weight: bold;">Low Expression</td>
					<td>&nbsp;</td>
					<td id="td_text_right" colspan="8" style="font-size:12px;font-weight: bold;">High Expression</td>
				</tr>
	...
	<%
		...
		for (SubsetExp subsetExp : gseInfoList) {				
			...
			if (geneName.indexOf("-") > 0) {
				...
				geneName1 = "<a href=\"javascript: searchGene('" + geneName1 + "', '" + type + "');\">" + geneName1 + "</a>";
		...
		<td align="left" colspan="7">
			...
			<div style="text-align:left;background-color: #555555;color: #ffffff;margin: 0 0 10px 0;padding: 3px 0 3px 5px;">
				...
				<%
					...
					if (start > 1) {
				%>
					<a style="color: black;text-decoration: none;" href="right_pvalue.jsp?subset1=${subset1}&subset2=${subset2}&isUpRegulation=${isUpRegulation}&pValue=${pValue}&start=<%= start - 20 %>&end=<%= start - 1 %>&type=${type}" onClick="searchPvalue();">&lt;</a>
				<%
					}
					...
					for (int i=startIndex, size=maxSize / 20 + addOne; i<size; i++) {
						...
						if (i != thisPage) {
				%>
						<a style="color: white;text-decoration: none;" href="right_pvalue.jsp?subset1=${subset1}&subset2=${subset2}&isUpRegulation=${isUpRegulation}&pValue=${pValue}&start=${indexStart}&end=${indexEnd}&type=${type}" onClick="searchPvalue();">${pageNumber}</a>
				<%
						} else {
				%>
				...
				<%
					...
					if (end  < maxSize) {
				%>
					<a style="color: black;text-decoration: none;" href="right_pvalue.jsp?subset1=${subset1}&subset2=${subset2}&isUpRegulation=${isUpRegulation}&pValue=${pValue}&start=<%= start + 20 %>&end=<%= start + 39 %>&type=${type}" onClick="searchPvalue();">&gt;</a>
				<%
					}
